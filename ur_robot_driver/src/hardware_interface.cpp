...
bool HardwareInterface::init(ros::NodeHandle& root_nh, ros::NodeHandle& robot_hw_nh)
{
  ...
  // The driver will offer an interface to receive complete trajectorise for executing on the robot
  ...
  try
  {
    ur_driver_.reset(
        new urcl::UrDriver(robot_ip_, script_filename, output_recipe_filename, input_recipe_filename,
                           ...
                           (uint32_t)script_sender_port, servoj_gain, servoj_lookahead_time, non_blocking_read_, reverse_ip, trajectory_port  ));
  }
